...
class FastCombination : DimGame() {
    ...
    fun onCraftItemEvent(event: CraftItemEvent) {
        ...
        println("productionItems.values : " + productionItems.values)

        if (productionItems.values.count { it == null } != 0) {
            return
        }
        ...
        println("resultMap.keys.filterNotNull().mapNotNull { Bukkit.getPlayer(it)?.displayName } : " + resultMap.keys.filterNotNull().mapNotNull { Bukkit.getPlayer(it)?.displayName })

        stopMiniGame(resultMap.keys.filterNotNull().mapNotNull { Bukkit.getPlayer(it) })
    }
